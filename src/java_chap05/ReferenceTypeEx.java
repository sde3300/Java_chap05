...
public class ReferenceTypeEx {
	...
	public static void main(String[] args) {
		...
		if (bValue == sValue) {
			System.out.println("bValued의 값과 sValue의 값은 같습니다.");
		}
		else {
			System.out.println("bValued의 값과 sValue의 값은 다릅니다.");
		}

		
		String str1 = "참조타입"; // new키워드를 사용해서 스택 영역과 힙 영역에 데이터를 추가
...
//		String 데이터 타입은 기본 타입처럼 사용하는 데이터 이므로 예외를 둬서 기본타입처럼 new 키워드 없이도 사용할 수 있도록 함
//		String 타입의 변수 2개 이상을 동일한 코드 블럭 내에서 선언하고 데이터를 new키워드 없이 대입연산자만 사용하여 저장했을 경우 먼저 선언된 String 타입의 변수에만 new 키워드가 사용되고, 나중에 선언된 String 타입의 변수에는 기존에 선언된 String 타입의 변수가 가지고 있는 메모리 주소를 대입함
